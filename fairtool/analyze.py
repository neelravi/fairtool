...
from .archive import band_gap_ev, get
...
def perform_single_file_analysis(data: dict, config: dict, identifier: str) -> dict:
    ...
    run = get(data, "run", 0)
    calculation = get(run, "calculation", -1)

    results = {
        "identifier": identifier,
        "total_energy_eV": _joule_to_ev(get(calculation, "energy", "total", "value")),
        "band_gap_eV": band_gap_ev(calculation),
        "converged": _scf_converged(calculation, get(run, "method", -1, "scf", "threshold_energy_change")),
    }
...
def _scf_converged(calculation, threshold) -> Optional[bool]:
    ...
    try:
        ...
        energies = [get(iteration, "energy", "free", "value") for iteration in get(calculation, "scf_iteration") or []]
